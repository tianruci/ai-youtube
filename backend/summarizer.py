...
- Form a complete content summary"""
    ...
    async def _integrate_hierarchical_summaries(self, chunk_summaries: list, target_language: str) -> str:
        """
        对大量分块摘要进行分层整合：先将 chunk_summaries 分组为小组，对每组调用 _integrate_chunk_summaries 得到组摘要，
        再将组摘要整合为最终摘要。为避免单次请求过长，分层整合可以降低上下文压力并提高鲁棒性。
        """
        try:
            ...
            # 逐组整合
            for idx in range(0, len(chunk_summaries), max_group_size):
                group = chunk_summaries[idx: idx + max_group_size]
                # 为每个子摘要添加标识以便整合时保留上下文顺序
                combined = "\n\n".join([f"[Part {idx + j + 1}]\n" + s for j, s in enumerate(group)])
                try:
                    group_summary = await self._integrate_chunk_summaries(combined, target_language)
                except Exception as e:
                    logger.error(f"整合分组摘要失败（组起始索引 {idx}）: {e}")
                    # 回退为简单拼接
            ...
            try:
                final_summary = await self._integrate_chunk_summaries(combined_groups, target_language)
                return final_summary
            except Exception as e:
                logger.error(f"最终分层整合失败: {e}")
                # 回退为按原顺序拼接所有局部摘要
                return "\n\n".join(chunk_summaries)

        except Exception as e:
            logger.error(f"分层整合摘要失败: {e}")
            return "\n\n".join(chunk_summaries)
